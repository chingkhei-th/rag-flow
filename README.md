...
├── data/                 # Directory containing raw input documents
├── chroma_db/            # Local SQLite-backed vector database storage
└── src/
    ├── config.py         # Logic for parsing and exporting environment variables
    ├── embeddings.py     # Custom Robust Embeddings wrapper for Ollama
    └── rag_pipeline.py   # Core Retrieval, Augmentation, and Generation logic
```
